...
function moocsIndicatorAlltime(jsondata) {
    var results = {
        ...
        porcent_total_downloads: (jsondata.length > 0) ? ((jsondata[0]['all_the_time_porcent_total_registrations'] * 100 >= 100) ? "100%" : (jsondata[0]['all_the_time_porcent_total_registrations'] * 100).toFixed(1) + '%') : '',
        /*missing, rodrigo dice que si falta el dato que pongamos 100% para all the time*/
...
function subscriberIndicatorAlltime(jsondata) {
    ...
    var results = {
        ...
        porcent_total_subscriber_2018: (jsondata.length > 0)&&(jsondata[0]['porcent_total_new_suscribers'])&& $('#idbLink')[0].text !== "IDB" ?  (checkDecimal(jsondata[0]['porcent_total_new_suscribers'] * 100))+'%': $('#idbLink')[0].text !== "IDB"?"0": '100%',
        /** missing, se pone 100% por orden de rodrigo */
...
function setSettingsNumber(valueNumber) {
    ...
    *********/
    ...
    if (valueNumber > 0 && valueNumber <= 999) {
        ...
        valueNumber = checkDecimal(valueNumber);
        suffixNumber = "";
