...
function getMinDate(item, data){
  var min = Math.min(...data.map(d=>d[item]));
  //console.log('min',min);
  return min;
}
function getMaxDate(item, data){
  var max = Math.max(...data.map(d=>d[item]));
  //console.log('max',max);
  return max;
}
...
function filterLACcountries(data,lacCheck){
  if(lacCheck){
    return data.filter(d => (lacCountries.indexOf(d.code3)>=0));
  }else{
    return data;
  }
}
...
function filterDropdown(data,selectedItem,item){
  if(selectedItem != 'all'){
    return data.filter(d => {return (d[item] == selectedItem);});
  }else{
    return data;
  }
}
...
function filterDate(item, data, from, to){
  ...
  var dataFilteredDownladDate = data
                                  .filter(d => {return (d[item] >= from)})
                                  .filter(d => {return (d[item] <= to)});
