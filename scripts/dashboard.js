...
function setDataIDBPublications() {
  $("#publication2018").prop("checked", true);
  var downloadTimelineIDB = $.extend(
    true,
    [],
    publicationsDownloadTimelineArray.downloadTimelineIDB
  );
  ...
  drawTreePublication(
    publicationsDownloadSourceArrays.downloadSourceIDB,
    "2018",
    "init"
  );
}
...
function SetDataIDBDataSet() {
  ...
  //timeline
  var ObjectDataSetLineChart = $.extend(
    true,
    [],
    datasetsDownloadsTimelineArrays.downloadsTimelineIDB
  );
...
function setDataPublicationsByDivisions(sltValue) {
  $("#publication2018").prop("checked", true);
  dataPublicationGauge2018 = $.extend(
    true,
    [],
  ...
  if (auxDownloadTimelineDivisions.length > 0) {
    auxDownloadTimelineDivisions = $.extend(
      true,
      [],
      auxDownloadTimelineDivisions[0].data
    );
  } else {
...
function setDataPublicationsByDepartment(sltValue) {
  ...
  auxDownloadTimelineDepartment = $.extend(
    true,
    [],
    auxDownloadTimelineDepartment[0].data
  );
...
function setDataMoocsByDivisions(sltValue) {
  ...
  drawMoocsRegistrationsChart(
    orderTopMoocs(divisionFilter(moocsTopArrays.divisions2018, sltValue))
  );
  drawDistributionChart(
    orderTopMoocs(
      divisionFilter(moocsEducationArrays.educationLevelDivisions2018, sltValue)
    )
  );
  var students = divisionFilter(
    moocsStudentsFlowArrays.studentsFlowDivisions,
    sltValue
  );
...
function setDataMoocsByDepartment(sltValue) {
  ...
  drawMoocsRegistrationsChart(
    orderTopMoocs(divisionFilter(moocsTopArrays.departments2018, sltValue))
  );
  drawDistributionChart(
    orderTopMoocs(
      divisionFilter(
        moocsEducationArrays.educationLevelDepartments2018,
        sltValue
      )
    )
  );
  var students = divisionFilter(
    moocsStudentsFlowArrays.studentsFlowDepartments,
    sltValue
  );
  drawStudentRegistrationsChart($.extend(true, [], students[0]));
  drawStudentParticipantsChart($.extend(true, [], students[0]));
...
function setDataDataSetByDivisions(sltValue) {
  ...
  //chart time line
  var ObjectDataSetTimeLine = $.extend(
    true,
    [],
  ...
  datasets2018TotalGlobal =
    jsondataDatasets.length > 0 ? jsondataDatasets[0]["2018_datasets"] : "0";
...
function setDataCodeByDivisions(sltValue) {
  ...
  var ObjectPageViewsTimeLine2018 = $.extend(
    true,
    [],
  ...
  var ObjectCodePageViewSource = $.extend(
    true,
    [],
...
function setDataSuscribersByDivisions(sltValue) {
  //treemap
  var ObjectGenderTreeMap = $.extend(
    true,
    [],
